...
/*!**************************************************!*\
  !*** ./assets/js/support/documentation-index.js ***!
  \**************************************************/
/**
...
(function (window, $, JTracker) {
  ...
  JTracker.loadDocumentationPage = function (fullPath) {
    $.ajax({
      ...
      complete: function complete() {
        $('#loading').hide(); // Resize the loading overlay
  ...
  JTracker.resizeDocumentationLoadingContainer = function () {
    var outerContainer = $('div.body > div.container'),
        ...
        loading = $('#loading');
    loading.css('top', pageSection.position().top - $(window).scrollTop()).css('left', pageSection.position().left).css('width', outerContainer.width()).css('height', outerContainer.height()).css('display', 'none');
  };
