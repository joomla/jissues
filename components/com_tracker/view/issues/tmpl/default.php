...
<form action="<?php echo htmlspecialchars(JUri::getInstance()->toString()); ?>" method="post" name="adminForm"
      id="adminForm" class="form-inline form-search">
	<?php if (!$this->project) : ?>
	...
	<?php else : ?>
		...
		<div class="filters btn-toolbar clearfix">
			<div class="filter-search btn-group pull-left input-append">
				<label class="filter-search-lbl element-invisible"
				       for="filter-search"><?php echo JText::_('COM_TRACKER_FILTER_SEARCH_DESCRIPTION'); ?></label>
				<input type="text" class="search-query input-xlarge" name="filter-search" id="filter-search"
				       ...
				       title="<?php echo JText::_('COM_TRACKER_FILTER_SEARCH_DESCRIPTION'); ?>"
				       placeholder="<?php echo JText::_('COM_TRACKER_FILTER_SEARCH_DESCRIPTION'); ?>"/>
				<button class="btn tip hasTooltip" type="submit"
				        title="<?php echo JText::_('JSEARCH_FILTER_SUBMIT'); ?>"><span class="icon-search"></span>
				</button>
			</div>
			<div class="btn-group pull-left">
				<button class="btn tip hasTooltip" type="button"
				        onclick="jQuery('#filter-search').val('');document.adminForm.submit();"
				        title="<?php echo JText::_('JSEARCH_FILTER_CLEAR'); ?>"><span class="icon-remove"></span>
				</button>
			</div>
			<div class="btn-group pull-left">
			...
			<div class="btn-group pull-right">
				<label for="status"
				       class="element-invisible"><?php echo JText::_('COM_TRACKER_FILTER_STATUS'); ?></label>
				<select name="filter-status" id="filter-status" class="input-medium"
				        onchange="document.adminForm.submit();">
		...
		<table class="table table-bordered table-striped">
			...
			<tr>
				...
				<th width="10%"
				    class="hidden-phone"><?php echo JText::_('COM_TRACKER_HEADING_LAST_MODIFIED'); ?></th>
			</tr>
			</thead>
			<tbody>
					...
					<tr <?php echo $rowClass; ?>>
						...
						<td class="hasContext">
							<div class="hasTooltip"
							     title="<?php echo JHtml::_('string.truncate', $this->escape($item->description), 100); ?>">
							...
							<?php if ($item->gh_id || $item->jc_id) : ?>
								<div class="small">
									<?php if ($item->gh_id) : ?>
										...
										<a href="https://github.com/<?= $this->project->gh_user . '/' . $this->project->gh_project ?>/issues/<?php echo (int) $item->gh_id; ?>"
										   target="_blank">
									...
									<?php if ($item->jc_id) : ?>
										...
										<a href="http://joomlacode.org/gf/project/joomla/tracker/?action=TrackerItemEdit&tracker_item_id=<?php echo (int) $item->jc_id; ?>"
										   target="_blank">
