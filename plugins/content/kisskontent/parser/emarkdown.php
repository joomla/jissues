...
class ElephantMarkdown
{
    ...
    public function runDocumentGamut($text)
    {
        ...
        $text = $this->doFencedCodeBlocks2($text);
	...
	public function runSpanGamut($text)
	{
		...
		$text = $this->doPlainLinks($text);

		$text = $this->doUserProjectSHA($text);
		$text = $this->doUserSHA($text);
		$text = $this->doSHA($text);

		$text = $this->doUserProjectIssues($text);
		$text = $this->doUserIssues($text);
		$text = $this->doIssues($text);
	...
	public function doPlainLinks($text)
	{
		$text = preg_replace_callback(
	...
	public function doSHA_callback($matches, $type = 'commit')
	{
	...
	public function doFencedCodeBlocks2($text)
	{
		#
		# Adding the fenced code block syntax to regular Markdown:
		#
		...
		$text = preg_replace_callback('{
			...
			array(&$this, '_doFencedCodeBlocks2_callback'), $text);

		return $text;
	}

	public function _doFencedCodeBlocks2_callback($matches)
	{
		if($matches[2])
		{
			...
			if(false == array_key_exists($matches[2], $knownLanguages))
			{
				$codeblock = "Unsupported syntax !\n\n".$matches[3];
				$codeblock = htmlspecialchars($codeblock, ENT_NOQUOTES);
				$codeblock = preg_replace_callback('/^\n+/',
					array(&$this, '_doFencedCodeBlocks_newlines'), $codeblock);
				$codeblock = "<pre><code>$codeblock</code></pre>";
			...
			else
			{
				...
				$caching = false;
				$codeblock = luminous::highlight($knownLanguages[$matches[2]], $matches[3], $caching);
			}
		}
		else
		{
